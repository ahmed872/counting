...
def main():
    ...
    def hr_has_no_direct_pdf_export_only_print():
        """A direct "حفظ PDF" (straight to file, no printer involved) used
        to sit next to طباعة on both the per-employee report and the full
        employee list. Reported live as still saving a blank page even
        after two independent fixes (an explicit QPrinter page size, then
        QPdfWriter instead of QPrinter entirely) - both worked in every
        environment available here except the one real machine that
        mattered. Printing and choosing a PDF-producing printer (e.g.
        "Microsoft Print to PDF") reproducibly worked, so the direct-export
        buttons and their handlers were removed rather than shipping a
        third guess - this pins that removal so it is not silently
        reintroduced later."""
        hr = window.hr
        assert not hasattr(hr, "save_employee_report_pdf")
        assert not hasattr(hr, "save_employee_list_pdf")
        assert hasattr(hr, "print_employee_report")
        assert hasattr(hr, "print_employee_list")
        goto("reports")
        assert not hasattr(window.reports, "save_pdf")
        assert hasattr(window.reports, "print_report")
    check("HR and Reports have no direct-to-file PDF export, only طباعة",
          hr_has_no_direct_pdf_export_only_print)
